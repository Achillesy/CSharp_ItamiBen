...
/// <summary>
/// 机芯的滴答声。**从 v3 原样搬过来**（只改了临时文件名和命名空间）。
///
/// ⚠️ **v4 只用「一直响」这一种**：v3 那边还有一种「跑偏才响」（`TickEnabled && _drifting`），
/// 那是个**跑偏提示音**——跟 DECISIONS D1「淡蓝块往环尾退本身就是惩罚，加提示等于
/// 承认视觉没说清楚」和 E4「偷懒不弹窗不出声」直接冲突，所以没搬。
/// 一直响的钟是**环境音**，不是提示，两者不是一回事。
///
...
public static class Tick
{
    ...
    private static string MacPath(bool tock)
        => Path.Combine(Path.GetTempPath(), tock ? "itamiben-tock.wav" : "itamiben-tick.wav");
