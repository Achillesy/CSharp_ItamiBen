...
///
/// 四个取值，但格子上只有两个计数（DECISIONS C7）：
/// <c>OffTask</c> / <c>Focused</c> 进 <c>sampledSeconds</c>，
/// <c>Unread</c> / <c>SelfExempt</c> **整秒丢掉**——既不计入也不算跑偏。
///
/// ⚠️ 分成四个而不是三个，只是为了日志里能看出「这一秒为什么没算」；
...
public static class Judgment
{
    ...
    /// 判一秒。
    ///
    /// 顺序是有讲究的，**自身豁免排在规则匹配前面**（见 <see cref="SecondOutcome.SelfExempt"/>）。
    /// </summary>
    ...
    public static SecondJudgment Judge(string app, string title, IReadOnlyList<string> goals, GoalRules rules)
    {
